...
from apps.grm.schemas import (CreateModuleIn, CreateModuleVarIn, GetModule,
                              GetModuleInfo, GetModuleVar, ReadValue, ReadValueIn,
                              UpdateModuleIn, WriteValueIn, WriteValueOut)
...
def get_grm_client(grm: GrmModule) -> GrmClient:
    ...
    key = (grm.module_id, grm.module_secret, grm.module_url,)
...
def write_local_var(model: GrmModuleVar, req: WriteValueIn):
    ...
    # 写入
    push_to_gateway(settings.PROMETHUES_PUSHGATEWAY, job='grm_local', registry=registry, grouping_key=labels)
...
def read_values(request,
    ...
    for module_var in var_list:
        ...
        query_resp = requests.get(f'{settings.PROMETHEUS_URL}/api/v1/query?{query_params}')
        ...
        for result in query_data['data']['result']:
            if result_type == 'vector':
                ...
                for value_set in result['values']:
                    values.append(ReadValue.Value(
                        timestamp=value_set[0],
                        value=float(value_set[1])))
...
def write_values(request, module_id: str, var_list: list[WriteValueIn]):
    ...
    for var_write in var_list:
        ...
        if not var_model.local:
            # 远程GRM设备
            grm_write_list = [ModuleVar(module_id=module_id,
                                        type=var_model.type,
                                        name=var_write.name,
